...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`ALTER TABLE \`exhibitions\` ADD \`featured_img_id\` text(36) REFERENCES media(id);`)
  await db.run(sql`CREATE INDEX \`exhibitions_featured_img_idx\` ON \`exhibitions\` (\`featured_img_id\`);`)
  await db.run(sql`ALTER TABLE \`_exhibitions_v\` ADD \`version_featured_img_id\` text(36) REFERENCES media(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_exhibitions\`("id", "start_date", "startdate_tz", "end_date", "enddate_tz", "location", "type", "generate_slug", "slug", "title", "press_release_id", "checklist_id", "preview_pdf_id", "cover_image_id", "meta_title", "meta_description", "meta_image_id", "updated_at", "created_at", "_status") SELECT "id", "start_date", "startdate_tz", "end_date", "enddate_tz", "location", "type", "generate_slug", "slug", "title", "press_release_id", "checklist_id", "preview_pdf_id", "cover_image_id", "meta_title", "meta_description", "meta_image_id", "updated_at", "created_at", "_status" FROM \`exhibitions\`;`)
