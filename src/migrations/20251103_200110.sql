  ...
  INSERT INTO `__new_exhibitions`("id", "start_date", "startdate_tz", "end_date", "enddate_tz", "location", "type", "generate_slug", "slug", "title", "press_release_id", "checklist_id", "preview_pdf_id", "cover_image_id", "featured_img_id", "meta_title", "meta_description", "meta_image_id", "updated_at", "created_at", "_status") SELECT "id", "start_date", "startdate_tz", "end_date", "enddate_tz", "location", "type", "generate_slug", "slug", "title", "press_release_id", "checklist_id", "preview_pdf_id", "cover_image_id", "featured_img_id", "meta_title", "meta_description", "meta_image_id", "updated_at", "created_at", "_status" FROM `exhibitions`;
  DROP TABLE `exhibitions`;
  ALTER TABLE `__new_exhibitions` RENAME TO `exhibitions`;
  PRAGMA foreign_keys=ON;
  CREATE UNIQUE INDEX `exhibitions_slug_idx` ON `exhibitions` (`slug`);
  CREATE INDEX `exhibitions_press_release_idx` ON `exhibitions` (`press_release_id`);
  CREATE INDEX `exhibitions_checklist_idx` ON `exhibitions` (`checklist_id`);
  CREATE INDEX `exhibitions_preview_pdf_idx` ON `exhibitions` (`preview_pdf_id`);
  CREATE INDEX `exhibitions_cover_image_idx` ON `exhibitions` (`cover_image_id`);
  CREATE INDEX `exhibitions_featured_img_idx` ON `exhibitions` (`featured_img_id`);
  CREATE INDEX `exhibitions_meta_meta_image_idx` ON `exhibitions` (`meta_image_id`);
  CREATE INDEX `exhibitions_updated_at_idx` ON `exhibitions` (`updated_at`);
  CREATE INDEX `exhibitions_created_at_idx` ON `exhibitions` (`created_at`);
  CREATE INDEX `exhibitions__status_idx` ON `exhibitions` (`_status`);
  CREATE TABLE `__new__exhibitions_v` (
  	`id` text(36) PRIMARY KEY NOT NULL,
  	`parent_id` text(36),
  ...
  INSERT INTO `__new__exhibitions_v`("id", "parent_id", "version_start_date", "version_startdate_tz", "version_end_date", "version_enddate_tz", "version_location", "version_type", "version_generate_slug", "version_slug", "version_title", "version_press_release_id", "version_checklist_id", "version_preview_pdf_id", "version_cover_image_id", "version_featured_img_id", "version_meta_title", "version_meta_description", "version_meta_image_id", "version_updated_at", "version_created_at", "version__status", "created_at", "updated_at", "latest") SELECT "id", "parent_id", "version_start_date", "version_startdate_tz", "version_end_date", "version_enddate_tz", "version_location", "version_type", "version_generate_slug", "version_slug", "version_title", "version_press_release_id", "version_checklist_id", "version_preview_pdf_id", "version_cover_image_id", "version_featured_img_id", "version_meta_title", "version_meta_description", "version_meta_image_id", "version_updated_at", "version_created_at", "version__status", "created_at", "updated_at", "latest" FROM `_exhibitions_v`;
  DROP TABLE `_exhibitions_v`;
  ALTER TABLE `__new__exhibitions_v` RENAME TO `_exhibitions_v`;
  CREATE INDEX `_exhibitions_v_parent_idx` ON `_exhibitions_v` (`parent_id`);
  CREATE INDEX `_exhibitions_v_version_version_slug_idx` ON `_exhibitions_v` (`version_slug`);
  CREATE INDEX `_exhibitions_v_version_version_press_release_idx` ON `_exhibitions_v` (`version_press_release_id`);
  CREATE INDEX `_exhibitions_v_version_version_checklist_idx` ON `_exhibitions_v` (`version_checklist_id`);
  CREATE INDEX `_exhibitions_v_version_version_preview_pdf_idx` ON `_exhibitions_v` (`version_preview_pdf_id`);
  CREATE INDEX `_exhibitions_v_version_version_cover_image_idx` ON `_exhibitions_v` (`version_cover_image_id`);
  CREATE INDEX `_exhibitions_v_version_version_featured_img_idx` ON `_exhibitions_v` (`version_featured_img_id`);
  CREATE INDEX `_exhibitions_v_version_meta_version_meta_image_idx` ON `_exhibitions_v` (`version_meta_image_id`);
  CREATE INDEX `_exhibitions_v_version_version_updated_at_idx` ON `_exhibitions_v` (`version_updated_at`);
  CREATE INDEX `_exhibitions_v_version_version_created_at_idx` ON `_exhibitions_v` (`version_created_at`);
  CREATE INDEX `_exhibitions_v_version_version__status_idx` ON `_exhibitions_v` (`version__status`);
  CREATE INDEX `_exhibitions_v_created_at_idx` ON `_exhibitions_v` (`created_at`);
  CREATE INDEX `_exhibitions_v_updated_at_idx` ON `_exhibitions_v` (`updated_at`);
  CREATE INDEX `_exhibitions_v_latest_idx` ON `_exhibitions_v` (`latest`);