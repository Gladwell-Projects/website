  ...
  return (
    <>
          ...
          return (
            ...
            >
              <Link
                ...
                className="text-(--theme-text) visited:text-(--theme-text) hover:text-shadow-(--text-outline)"
              >
                {artist.title}
              </Link>
            </li>
      ...
      <div className="pointer-events-none invisible fixed inset-0 z-99 col-span-full hidden h-full w-full object-contain md:visible md:block">
        {data.map((artist) => {
          const cover = isRenderableImage(artist.cover)
            ? artist.cover
            : isRenderableImage(artist.profileImage)
              ? artist.profileImage
              : isRenderableImage(artist.surveyArtworks[0])
